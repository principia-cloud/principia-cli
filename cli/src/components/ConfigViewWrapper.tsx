...
export const ConfigViewWrapper: React.FC<ConfigViewWrapperProps> = ({
	...
	const handleOpenFolder = useCallback(
		async (folderType: "rules" | "workflows" | "hooks" | "skills", isGlobal: boolean) => {
			...
			if (isGlobal) {
				// Global folders are in dataDir (e.g., ~/.principia/)
				...
				// Local rules/workflows/hooks/skills are in .principiarules or .cline
				const subFolder = folderType === "rules" ? "rules" : folderType
				folderPath = path.join(primaryWorkspace, ".principiarules", subFolder)
			}

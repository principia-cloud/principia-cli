...
// ASCII art Principia logo (scaled robot + centered block text)
const PRINCIPIA_WELCOME_LOGO = [
	"                    █████                    ",
	"                █████████████                ",
	"              █████████████████              ",
	"             ███████████████████             ",
...
const WELCOME_LOGO_GRADIENT_COLORS = [
	"#00D9FF",
	"#00CBFF",
	"#00B8FF",
	"#1DA5FF",
...
export const WelcomeView: React.FC<WelcomeViewProps> = ({ onSubmit, onExit, controller }) => {
	...
	return (
		<Box flexDirection="column" width="100%">
			...
			{/* Principia logo - centered with gradient */}
			<Box alignItems="center" flexDirection="column">
				{PRINCIPIA_WELCOME_LOGO.map((line, idx) => (
					// biome-ignore lint/suspicious/noArrayIndexKey: static array that never changes
					<Text color={WELCOME_LOGO_GRADIENT_COLORS[idx] || "#FF3FFF"} key={idx}>
						{line}
					</Text>
				))}
			</Box>

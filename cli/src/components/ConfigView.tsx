...
export const ConfigView: React.FC<ConfigViewProps> = ({
	...
	const ruleEntries = useMemo(() => {
		...
		entries.push(...buildToggleEntries(globalClineRulesToggles, "global", "principia"))
		entries.push(...buildToggleEntries(localClineRulesToggles, "workspace", "principia"))
	...
	const handleToggle = () => {
		if (currentTab === "rules" && ruleEntries[selectedIndex] && onToggleRule) {
			...
			onToggleRule(entry.source === "global", entry.path, !entry.enabled, entry.ruleType || "principia")
			...
			case "rules": {
				if (ruleEntries.length === 0) {
					return (
						<Box>
							<Text color="gray">
								No rules configured. Add .principiarules files to your workspace or global config.
							</Text>
						</Box>
					)
				}

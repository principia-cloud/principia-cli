...
import { PRINCIPIA_CLI_DIR, getCliBinaryPath } from "./utils/path"
...
async function initializeCli(options: InitOptions): Promise<CliContext> {
	...
	const outputChannel = window.createOutputChannel("Principia CLI")
	...
	outputChannel.appendLine(
		`Principia CLI initialized. Data dir: ${DATA_DIR}, Extension dir: ${EXTENSION_DIR}, Log dir: ${PRINCIPIA_CLI_DIR.log}`,
	)
...
program.name("principia").description("Principia - open-source CLI for building robotics simulations from natural language").version(CLI_VERSION)
...
program
	...
	.option("--config <path>", "Path to Principia configuration directory")
...
program
	.command("version")
	.description("Show Principia CLI version number")
	.action(() => printInfo(`Principia CLI version: ${CLI_VERSION}`))

...
export function initializeCliContext(config: CliContextConfig = {}): CliContextResult {
	const PRINCIPIA_DIR = config.clineDir || process.env.PRINCIPIA_DIR || path.join(os.homedir(), ".principia")
	const DATA_DIR = path.join(PRINCIPIA_DIR, SETTINGS_SUBFOLDER)

	// Workspace storage should always be under ~/.principia/data/workspaces/<hash>/

...
def export_layout_to_mesh_dict_list(layout: FloorPlan):
    ...
    def get_object_mesh(source, source_id, layout_id):
        for ext in (".ply", ".obj"):
            object_save_path = f"{layout_save_dir}/{layout_id}/{source}/{source_id}{ext}"
    ...
    for room in layout.rooms:
        ...
        for window_id, window_mesh in zip(room_window_ids, room_window_meshes):
            window_mesh_tex_coords = create_wall_mesh_tex_coords(window_mesh)
    ...
    for room in layout.rooms:
        ...
        for obj in room.objects:
            ...
            if obj_mesh is not None:
                ...
                texture_info = get_object_mesh_texture(obj.source, obj.source_id, layout.id, mesh=obj_mesh)
    ...
    for room in layout.rooms:
        ...
        for door in room.doors:
            ...
            door_mesh_tex_coords = create_wall_mesh_tex_coords(door_mesh)
            ...
            if not os.path.exists(door_frame_texture_map_path):
                door_frame_texture_map_path = door_mesh_texture_map_path

            door_frame_tex_coords = create_wall_mesh_tex_coords(door_frame_mesh)
...
def export_single_room_layout_to_mesh_dict_list_from_room(room: Room, layout_id: str):
    ...
    for obj in room.objects:
        ...
        if obj_mesh is not None:
            # Apply transforms to the object mesh
            transformed_mesh = apply_object_transform(obj_mesh, obj)
            texture_info = get_object_mesh_texture(obj.source, obj.source_id, layout_id, mesh=obj_mesh)
...
def get_single_object_mesh_info_dict(scene_save_dir, source, source_id):
    ...
    def get_object_mesh(source, source_id):
        for ext in (".ply", ".obj"):
            object_save_path = f"{scene_save_dir}/{source}/{source_id}{ext}"
            if os.path.exists(object_save_path):
                return trimesh.load(object_save_path)
        return None
    ...
    if obj_mesh is not None:
        texture_info = get_object_mesh_texture(source, source_id, mesh=obj_mesh)

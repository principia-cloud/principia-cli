...
# Usage: curl -fsSL https://principia.cloud/install.sh | bash
...
build() {
    ...
    (cd "$source_dir" && npm install --no-audit --no-fund --loglevel=error) \
...
setup_kb() {
    ...
    if [ -d "${kb_dir}/simulator_kb.lance" ]; then
        ...
        curl -fsSL "$download_url" | tar xz -C "$kb_dir" --exclude='._*' \
...
main() {
    ...
    printf '  \033[1;38;5;69m██╔═══╝ ██╔══██╗██║██║╚██╗██║██║     ██║██╔═══╝ ██║██╔══██║\033[0m\n'
    printf '  \033[1;38;5;141m██║     ██║  ██║██║██║ ╚████║╚██████╗██║██║     ██║██║  ██║\033[0m\n'
    printf '  \033[1;38;5;135m╚═╝     ╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝ ╚═════╝╚═╝╚═╝     ╚═╝╚═╝  ╚═╝\033[0m\n'
    printf '\n'
    printf '                        \033[1;38;5;45mAgent Installer\033[0m\n\n'

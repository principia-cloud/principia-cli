...
class ClineEndpoint {
	...
	private constructor() {
		...
		const _env = process?.env?.PRINCIPIA_ENVIRONMENT_OVERRIDE || process?.env?.PRINCIPIA_ENVIRONMENT
	 ...
	 * Located at ~/.principia/endpoints.json
	 */
	private static getEndpointsFilePath(): string {
		return path.join(os.homedir(), ".principia", "endpoints.json")
	}
	 ...
	 * Priority: bundled endpoints.json → ~/.principia/endpoints.json → null (standard mode)
	...
	private static async loadEndpointsFile(): Promise<EndpointsFileSchema | null> {
		...
		// 2. Try ~/.principia/endpoints.json
	...
	public setEnvironment(env: string) {
		if (this.onPremiseConfig) {
			throw new Error("Cannot change environment in on-premise mode. Endpoints are configured via ~/.principia/endpoints.json")
		}

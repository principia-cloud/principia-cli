...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".principia", "skills")
...
Instructions here`)
		...
		it("should discover skills from project .principiarules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".principiarules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .principia/skills directory", async () => {
			const clineSkillsDir = path.join(TEST_CWD, ".principia", "skills")

	...
	if (await fileExistsAtPath(globalClineRulesFilePath)) {
		if (await isDirectory(globalClineRulesFilePath)) {
			try {
				...
				Logger.error(`Failed to read .principiarules directory at ${globalClineRulesFilePath}`)
			}
		} else {
	...
	if (await fileExistsAtPath(clineRulesFilePath)) {
		if (await isDirectory(clineRulesFilePath)) {
			try {
				...
				Logger.error(`Failed to read .principiarules directory at ${clineRulesFilePath}`)
			}
		} else {
			try {
				if (clineRulesFilePath in toggles && toggles[clineRulesFilePath] !== false) {
					...
					if (raw) {
						// Keep single-file .principiarules behavior consistent with directory/remote rules:
				...
				Logger.error(`Failed to read .principiarules file at ${clineRulesFilePath}`)
			}
		}
	}

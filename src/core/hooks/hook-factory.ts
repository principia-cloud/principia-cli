 ...
 * and workspace-specific hooks (from each workspace's .principiarules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .principiarules that they don't want to execute
...
export class HookFactory {
	 ...
	 * - Workspace hooks (from workspaceRoot/.principiarules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.principiarules/hooks/
	 ...
	 * Workspace hooks are located in workspace .principiarules/hooks/ directories
	 *
	 ...
	 * (from .principiarules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .principiarules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .principiarules directory path to search

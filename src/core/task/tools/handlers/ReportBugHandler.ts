	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Principia wants to create a github issue...",
				message: `Principia is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				await createAndOpenGitHubIssue("principia-cloud", "principia-agent", "bug_report.yml", params)
			} catch (error) {

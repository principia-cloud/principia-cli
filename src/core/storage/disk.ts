...
export const GlobalFileNames = {
	...
	clineruleSkillsDir: ".principiarules/skills",
	clineSkillsDir: ".principia/skills",
...
/**
 * Returns the cross-platform path to the Cline home directory (~/.principia).
...
export function getClineHomePath(): string {
	return path.join(os.homedir(), ".principia")
}
...
/**
 * Returns the global skills directory path (~/.principia/skills).
 ...
 * 2. Each workspace root's .principiarules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .principiarules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .principiarules/hooks folder in this workspace root.
